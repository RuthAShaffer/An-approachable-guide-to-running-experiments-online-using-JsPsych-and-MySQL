...
jsPsych.plugins['survey-text-subjectID'] = (function() { // RAS changed name of plugin call 
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      // add text box
      $("#jspsych-survey-text-" + i).append('<textarea name="#jspsych-survey-text-response-' + i + '" cols="' + trial.columns[i] + '" rows="' + trial.rows[i] + '" class = subjectID></textarea>'); //RAS added class = subjectID so can access more easily in css
